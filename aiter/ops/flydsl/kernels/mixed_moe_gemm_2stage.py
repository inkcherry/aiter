...
def compile_mixed_moe_gemm2(
    *,
    ...
    k_batch:
      - 1 (default): legacy single-K-loop -- each CTA processes the full
        inter_dim K range for its (M, N) tile.
      - >1: **split-K mode** -- the K dimension (inter_dim) is split into
        ``k_batch`` equal chunks along grid.z. Each CTA processes
        ``inter_dim / k_batch`` K-elements and atomically accumulates its
        partial result into the output. Increases in-flight blocks by
        ``k_batch``x, which can improve latency hiding for memory-bound
        workloads. Requires ``accumulate=True`` (atomic fadd path) and
        ``inter_dim`` divisible by ``k_batch * tile_k * 2`` (even number of
        K-tiles per batch so the 2-tile tail path is always safe).
    ...
    n_per_block:
      - 1 (default): legacy behavior, each CTA handles one N-tile.
      - >1: **N-tile reuse mode** -- each CTA handles `n_per_block` consecutive
        N-tiles back-to-back while keeping the same M-tiles. This amortizes A
        (input activation) L2/DRAM loads across the N-tiles and can reduce
        W2 L2 thrashing when the concurrent expert working set exceeds L2.
        Requires (model_dim / tile_n) % n_per_block == 0.
    ...
    # Round 50 (Team A): empirical override for the production prefill stage2
    # path on MI355X. The "_w4" registry advertises waves_per_eu=4 but the
    # t64x128x256 atomic+persist+async fp4xfp4 path benches faster with
    # waves_per_eu=6 (median ~3091us vs ~3117us at M=32768 prefill).
    # Standalone w6 sweep: w4=3117, w5=3101, w6=3091, w8=3097.
    # Gated to the exact production tile/dtype combo so other registry
    # entries are unaffected.

    # Team D r75: gate wpe=4->6 override to cu_num_mul==1 only.
    # With cumul>1 the grid is already larger, so wpe=6 may push
    # occupancy too high and increase contention.
    if (
        waves_per_eu == 4
        and tile_m == 64 and tile_n == 128 and tile_k == 256
        and persist_m <= 0 and use_async_copy
        and a_dtype == "fp4" and b_dtype == "fp4"
        and k_batch == 1 and n_per_block == 1
        and group_size_m == 1 and w_nt == 0
        and cu_num_mul <= 1
    ):
        waves_per_eu = 6

    # R139: issue async X DMA early in prologue for max overlap with B/scale VMEM.
    ...
    # cu_num_mul (Team D r45, ported from Team B r683): multiplies the
    # persistent-mode grid_y (CU count) by an integer factor. cu_num_mul=1
    # (default) is byte-identical to production. Values >1 launch MORE
    # persistent CTAs, each covering FEWER M-tiles, increasing in-flight
    # parallelism. Team B measured cu_num_mul=4 as -2.4% on MI355X.
    ...
    _npb_tag = f"_npb{n_per_block}" if n_per_block > 1 else ""
    _npbX_tag = f"_npbX{npb_inner}" if npb_inner > 1 else ""
    _wpe_tag = f"_w{waves_per_eu}" if waves_per_eu is not None else ""
    if n_per_block < 1:
        raise ValueError(f"n_per_block must be >= 1, got {n_per_block}")
    if npb_inner < 1:
        raise ValueError(f"npb_inner must be >= 1, got {npb_inner}")
    if npb_inner > 1 and n_per_block > 1:
        raise ValueError(
            "npb_inner>1 and n_per_block>1 cannot be combined; pick one."
        )
    if npb_inner > 1 and (int(model_dim) // int(tile_n)) % int(npb_inner) != 0:
        raise ValueError(
            f"npb_inner={npb_inner}: (model_dim/tile_n)={int(model_dim)//int(tile_n)}"
            f" must be divisible by npb_inner."
        )
    ...
    if _is_splitk:
        if not accumulate:
            raise ValueError(
                "k_batch>1 requires accumulate=True (atomic fadd path is the "
                "only safe way to merge partial-K results)."
            )
        if int(inter_dim) % int(k_batch) != 0:
            raise ValueError(
                f"inter_dim={inter_dim} not divisible by k_batch={k_batch}"
            )
        _k_per_batch = int(inter_dim) // int(k_batch)
        if _k_per_batch % int(tile_k) != 0:
            raise ValueError(
                f"inter_dim/k_batch={_k_per_batch} not divisible by "
                f"tile_k={tile_k}"
            )
        _num_k_tiles_per_batch = _k_per_batch // int(tile_k)
        # We need the 2-tile tail path to be valid: at least 2 K-tiles per
        # batch. (odd_k_tiles=1 path works for 1 K-tile but split-K with
        # num_tiles=1 is not useful since k_main2 is empty.)
        if _num_k_tiles_per_batch < 2:
            raise ValueError(
                f"k_batch={k_batch} leaves only {_num_k_tiles_per_batch} "
                f"K-tile(s) per batch (need >=2 for the split-K tail path); "
                f"reduce k_batch."
            )
        # Require even K-tile count per batch so the (2-tile tail, even main)
        # path is always taken -- avoids complicating the odd-tail handling
        # with per-batch K bounds.
        if _num_k_tiles_per_batch % 2 != 0:
            raise ValueError(
                f"k_batch={k_batch} gives odd K-tile count "
                f"({_num_k_tiles_per_batch}) per batch. Pick k_batch so that "
                f"inter_dim/(k_batch*tile_k) is even."
            )
        # Scale pack index stride per batch (must be integral).
        _scale_k_period = int(_scale_pack_k) * 128  # K-elements per scale pack
        if _k_per_batch % _scale_k_period != 0:
            raise ValueError(
                f"inter_dim/k_batch={_k_per_batch} not divisible by scale "
                f"pack K period={_scale_k_period}; pick a smaller k_batch."
            )
        _bz_scale_pack_step = _k_per_batch // _scale_k_period
    else:
        _k_per_batch = int(inter_dim)
        _num_k_tiles_per_batch = _k_per_batch // int(tile_k)
        _bz_scale_pack_step = 0
    _sk_tag = f"_sk{k_batch}" if _is_splitk else ""
    # ---- Raster/threadblock-swizzle validation ----
    if not isinstance(group_size_m, int) or group_size_m < 1:
        raise ValueError(f"group_size_m must be int >= 1, got {group_size_m}")
    _has_raster = group_size_m > 1 and _persistent
    if group_size_m > 1:
        if not _persistent:
            raise ValueError(
                "group_size_m>1 is only supported in persistent mode (persist_m<=0)"
            )
        if _cu_num % group_size_m != 0:
            raise ValueError(
                f"group_size_m={group_size_m} does not divide cu_num={_cu_num}; "
                f"pick a divisor of cu_num."
            )
    _gm_tag = f"_gm{group_size_m}" if _has_raster else ""
    _async_tag = "_async" if use_async_copy else ""
    _wnt_tag = f"_wnt{int(w_nt)}" if int(w_nt) != 0 else ""
    _cumul_tag = f"_cumul{int(cu_num_mul)}" if int(cu_num_mul) != 1 else ""
    module_name = (
        f"mfma_moe2_a{a_dtype}_w{b_dtype}_{out_s}_{epilog_tag}"
        f"_t{tile_m}x{tile_n}x{tile_k}"
        f"_vscale_fix31_twvec4_r50wpe6_r57ldstw_r58mergeIf_r85bsp3v2_r90dxasFinal_r139xdmaprol_r216tidDeferAfterB_r238bufPreValid_tD4balPersist_tD4xaddrPhys_tD14dswrAsync{_pm_tag}{_sbm_tag}{_npb_tag}{_npbX_tag}{_wpe_tag}{_sk_tag}{_gm_tag}{_async_tag}{_wnt_tag}{_cumul_tag}"
    ).replace("-", "_")
    ...
    lds_tid_bytes = int(tile_m) * 4
    # Round 57 (Team A): also stage sorted topk weights into LDS once per
    ...
    if True:
        ...
        def moe_gemm2(
            ...
            tx = gpu.thread_id("x")
            # by_outer is the raw block_id.x. When n_per_block > 1 each block
            # handles `n_per_block` consecutive N-tiles starting at by_outer *
            # n_per_block. The inner `_moe_gemm2_then_body` closure captures
            # `by` from this scope -- the scheduler at the call site below
            # rebinds `by` before each invocation so the body sees the correct
            # N-tile id for this iteration.
            # ---- Threadblock swizzle (group_size_m > 1) ---------------------
            # Default grid is (gx=n_blocks, gy=cu_num); block_id.x is the raw
            # N-tile and block_id.y is the raw CU-m index. When group_size_m>1
            # we remap the (bx, by) pair so that consecutive linear block IDs
            # stay on the SAME N-tile for ``group_size_m`` iterations before
            # advancing to the next N-tile. This clusters CUs onto a small set
            # of N-tiles at any instant, shrinking the W2 L2 working set and
            # raising the L2 hit rate on memory-bound prefill workloads.
            #
            #   linear_id  = bx_raw + gx * by_raw       (bx varies fastest)
            #   group_id   = linear_id // group_size_m
            #   group_off  = linear_id %  group_size_m
            #   by_outer   = group_id % gx              (N-tile index)
            #   by_cu_row  = group_id // gx             (CU-row within gy)
            #   bx_persist = by_cu_row * group_size_m + group_off
            #
            # Requires cu_num % group_size_m == 0 (validated at compile time).
            _npb_grid_factor = int(n_per_block) * int(npb_inner)
            if _has_raster:
                _gx_for_raster = n_in // arith.constant(
                    tile_n * _npb_grid_factor, index=True
                )
                _c_gsm = arith.constant(int(group_size_m), index=True)
                _lin = gpu.block_id("x") + _gx_for_raster * gpu.block_id("y")
                _grp_id = _lin // _c_gsm
                _grp_off = _lin % _c_gsm
                by_outer = _grp_id % _gx_for_raster
                _by_cu_row = _grp_id // _gx_for_raster
                bx_persist = _by_cu_row * _c_gsm + _grp_off
            else:
                by_outer = gpu.block_id("x")
                bx_persist = gpu.block_id("y")  # persistent WG index (M-dim)
            by = by_outer  # default: n_per_block == 1

            # ---- Split-K: each grid.z slice processes a different K range ----
            if _is_splitk:
                bz = gpu.block_id("z")
                # K-element offset for this batch along inter_dim.
                _c_k_per_batch = arith.constant(_k_per_batch, index=True)
                _bz_k_off_elems = bz * _c_k_per_batch
                # Scale pack index offset (constant per-batch step in
                # scale-pack units: _k_per_batch / (_scale_pack_k*128)).
                _c_bz_scale_step = arith.constant(_bz_scale_pack_step, index=True)
                _bz_scale_off = bz * _c_bz_scale_step
            else:
                _bz_k_off_elems = arith.index(0)
                _bz_scale_off = arith.index(0)
            ...
            # Round 57 (Team A): lds_tw aliases the LDS slot immediately after
            ...
            if _persistent:
                # Expert-phase scheduling: balanced contiguous M-tile dispatch.
                # Each CTA gets floor(T/CU) or ceil(T/CU) tiles, distributing
                # the remainder across the first (T % CU) CTAs.  This avoids
                # a tail band of CTAs that enter the persist loop with no valid
                # work (old ceil mapping).  Same expert-contiguous ordering is
                # preserved: adjacent tiles => same expert => B weight L2 reuse.
            ...
            def _moe_gemm2_then_body():
                ...
                # xaddrPhys: when use_async_copy is active with packed-elem
                # (fp4) A, the DMA instruction (raw_ptr_buffer_load_lds) uses
                # only `_num_dma_loads` address slots (typically 2), not the
                # full `num_x_loads` (typically 4).  Decode only the physically
                # needed rows to avoid 2 unnecessary VMEM buffer_load +
                # decode chains per thread in the prologue.
                if use_async_copy and a_elem_vec_pack > 1:
                    _dma_bytes_pre = 16
                    _eff_bytes_pre = (
                        int(tile_m) * int(_eff_lds_stride) * int(a_elem_bytes)
                ...
                def load_b_packs_k64(
                    ...
                    list (used by the K-outer/N-inner pipeline that maintains
                    one list per ``npb_inner`` slot). When ``None`` we fall
                    back to the body-level defaults (``n_blk_list`` /
                    ``n_intra_list``) so the legacy path stays bit-identical.
                    """
                ...
                # Round 85 (Team A): for the production prefill shape
                # (tile_m=64, tile_n=128, tile_k=256, fp4, async),
                # shift the b_lo/b_hi split asymmetrically so b_lo loads
                # 3/4 of K and b_hi only 1/4.  Rationale: the original
                # _b_split_ku = k_unroll // 2 (= 2 of 4) issues a sizeable
                # b_hi VMEM burst right before s_setprio(1) and the K-loop,
                # which can stall the first pack_M*pack_N MFMAs waiting on
                # VMEM issue slots.  With _b_split_ku = k_unroll - 1 (= 3
                # of 4) the b_hi burst is one ku worth (~1/4 of B) and the
                # extra ku of b_lo is issued in the prologue where it
                # overlaps with the X DMA (the dominant prologue latency).
                # The b_hi data is still needed at k_idx == _b_split_ku, by
                # which time the K-loop has issued 3 kus of MFMAs (~96
                # MFMAs for tile_m=64,tile_n=128) which is enough latency
                # to hide a 1-ku VMEM round-trip.
                _is_prefill_shape = (
                    int(tile_m) == 64
                    and int(tile_n) == 128
                    and int(tile_k) == 256
                ...
                def load_b_scale_tile(base_k, k_shift_bits=0, *, by_n_p=None):
                    ...
                    `by_n_p` overrides the body-default `by_n` so the
                    K-outer/N-inner pipeline (npb_inner>1) can request the
                    scale for a different N-tile while reusing this helper.
                    """
                ...
                # --- Async X-copy (buffer_load_lds): DMA X tile directly
                # from gmem into LDS, bypassing per-thread register staging.
                # Mirrors stage1's use_async_copy path but adapted for
                # stage2's single `lds_x` memref with ping/pong element
                # offsets (stage1 uses separate memrefs per buffer).
                if use_async_copy:
                    _dma_bytes = 16
                    _wave_size = 64
                    _eff_bytes_per_buffer = (
                ...
                def compute_tile(
                    ...
                    *,
                    ...
                    if prefetch_epilogue:
                        ...
                        tw_pf = None
                        if doweight_stage2:
                            # Round 57 (Team A): when lds_tw is staged in the
                            # prologue (default for the production prefill
                            # kernel), prefetch the topk-weight tile via vec4
                            # LDS reads instead of per-mi VMEM dwordx4 loads.
                            # This eliminates m_repeat VMEM ops from
                            # compute_tile (4 ops for tile_m=64), shaving issue
                            # slots that the MFMA-heavy hot loop can otherwise
                            # use.  LDS read latency (~10 cyc) << VMEM (~500),
                            # so the values are ready well before the cshuffle
                            # epilogue consumer.  Falls back to Round 13's vec4
                            # VMEM path when lds_tw is unavailable.
                            tw_pf = []
                            lane_div_16_mul4_pf = lane_div_16 * arith.index(4)
                    ...
                    # Round 90 (Team A): emit AMDGPU `disable_xdl_arb_stall`
                    # marker BEFORE the b_hi prefetch and s_setprio(1). On
                    # gfx950 the XDL arbiter normally throttles MFMA issue
                    # when sibling waves on the same SIMD also have MFMA in
                    # flight; disabling that here lets our high-prio K-loop
                    # drain MFMAs back-to-back. Gated to the production
                    # prefill shape so other kernels keep default scheduler.
                    _xdl_arb_hint = (
                        int(tile_m) == 64
                        and int(tile_n) == 128
                        and int(tile_k) == 256
                    ...
                    for k_idx in range_constexpr(k_unroll):
                        ...
                        for mi in range_constexpr(m_repeat_packed):
                            ...
                            # Hoist A-side LDS loads + a128 pack OUT of the ni
                            # loop. Address depends only on (k_idx, mi, imxdl),
                            # so previously the same ds_read was emitted
                            # `num_acc_n_packed` times per (mi, imxdl) tuple.
                            # Even with LLVM CSE that potentially leaves
                            # redundant value-build ops; emitting once removes
                            # the redundant MLIR/LLVM IR and makes the A-side
                            # LDS reads issue back-to-back at the head of the
                            # mi block where their latency can overlap the
                            # following ni*MFMA chain.
                            ...
                            for ni in range_constexpr(num_acc_n_packed):
                                ...
                                for imxdl in range_constexpr(pack_M):
                                    ...
                                    for inxdl in range_constexpr(pack_N):
                                        ...
                                        acc_idx = mi_idx * num_acc_n + ni_idx
                                        # NOTE: removed per-MFMA `sched_barrier(0)` here.
                                        # In combination with `s_setprio(1)` around
                                        # the MFMA loop, this lets the LLVM backend
                                        # interleave MFMAs with the next b_hi VMEM
                                        # loads and DSREADs, improving overlap.
                                        acc_list[acc_idx] = (
                                            rocdl.mfma_scale_f32_16x16x128_f8f6f4(
                                                mfma_res_ty,
                                                [
                    ...
                    # NOTE: previously dropped to s_setprio(0) here. We now
                    # keep prio=1 across the post-compute gpu.barrier and the
                    # VMEM-heavy prefetch_x_to_lds + scale prefetch phases of
                    # the K-loop pipeline, so this wave is consistently
                    # favored on the EU vs sibling WGs that may be in their
                    # own MFMA loops. The next compute_tile's `s_setprio(1)`
                    # then becomes a no-op; the cshuffle epilog explicitly
                    # bumps to prio 3 and drops to 0 at end.
                    return acc_list, epilogue_pf

                # ---- Per-ni epilogue (used only by npb_inner>1 path) ----
                # We duplicate the legacy epilogue body inside a Python loop
                # over `ni`, rebinding the closure-captured `acc` / `by_n`
                # locals per iteration so the cshuffle helpers see per-ni
                # state. The legacy epilogue at the bottom of this body is
                # untouched (npb_inner==1 keeps the bit-identical path).
                def _run_per_ni_epilogue_inline(
                    *,
                    npb_inner,
                    acc_per_ni,
                    by_list,
                    ...
                    if lds_out is None:
                        raise RuntimeError(
                            "FLIR_MOE_STAGE2_CSHUFFLE=1 but lds_out is not "
                            "allocated/aliased."
                        )
                    ...
                    out_elem_bytes_l = 4 if out_is_f32 else 2
                    _e_vec = (
                        2 if accumulate else min(tile_n // 32, 8)
                    )

                    def _idx_to_llvm_ptr(idx_val, addr_space=1):
                        idx_v = (
                            idx_val._value
                            if hasattr(idx_val, "_value")
                            else idx_val
                        )
                        i64_v = arith.index_cast(T.i64, idx_v)
                        i64_raw = (
                            i64_v._value
                            if hasattr(i64_v, "_value")
                            else i64_v
                        )
                        ptr_ty = ir.Type.parse(
                            f"!llvm.ptr<{addr_space}>"
                        )
                        return llvm.inttoptr(ptr_ty, i64_raw)

                    for _ni_inner in range_constexpr(int(npb_inner)):
                        # Per-ni rebinding (Python locals shadow body names).
                        _acc = acc_per_ni[_ni_inner]
                        _by_n = by_n_list[_ni_inner]
                        # Optional prefetched bias / topk weights are not
                        # issued by the new pipeline (we ran the K-loop
                        # without prefetch_epilogue=True). The closures
                        # below fall back to inline buffer_loads where
                        # needed (works for our enable_bias=False cfg).
                        tw_pf = None
                        bias_pf = None
                        ...
                        def precompute_row(*, row_local, row):
                            ...
                            t_idx = arith.index_cast(
                                ir.IndexType.get(), t
                            )
                            s_idx = arith.index_cast(
                                ir.IndexType.get(), s
                            )
                            ts_idx = t_idx * arith.constant(
                                topk, index=True
                            ) + s_idx
                            if accumulate:
                                row_byte_base = (
                            ...
                            *,
                            ...
                            if not bool(accumulate):
                                llvm.StoreOp(
                                    frag_v,
                                    out_ptr_v,
                                    alignment=_e_vec * out_elem_bytes_l,
                                    nontemporal=True,
                                )
                            else:
                                llvm.AtomicRMWOp(
                        ...
                        # Inter-ni LDS reuse: the cshuffle epilogue uses
                        # lds_out (an alias of the same bytes as A LDS).
                        # We need a barrier so the next ni doesn't start
                        # writing lds_out before the prior ni's atomic
                        # stores have finished reading from it.
                        gpu.barrier()

                # ====================================================================
                # Local copies of _k_base / _k_shift_bits so the K-outer/N-inner
                # path below (and the legacy path further down) both see them.
                # The originals are re-defined later in the legacy block; that
                # is harmless (closure shadowing inside the branch).
                # ====================================================================
                ...
                # ====================================================================
                # NEW pipeline: K-outer / N-inner (npb_inner > 1)
                #
                # Goal: cut A2 (X) and A-scale DRAM traffic per CTA by serving
                # ``npb_inner`` consecutive N-tiles from the SAME LDS A tile per
                # K-step instead of re-running the K-loop per N-tile (the
                # outer-N path that the existing scf.for n_per_block does).
                #
                # For each K-step we:
                #   1. Load X  -> regs              (N-INDEPENDENT, 1x)
                #   2. Load A-scale prefetch        (N-INDEPENDENT, 1x)
                #   3. For each ni in [0, npb_inner):
                #        load_b_tile_lo for ni      (N-DEPENDENT, npb_inner x)
                #        load_b_scale_tile for ni   (N-DEPENDENT, npb_inner x)
                #   4. Compute compute_tile(acc[ni], b[ni], lds_pong, ...)
                #      for each ni, with per-ni acc + per-ni n_scale_shift,
                #      reusing the SAME LDS A.
                #   5. Store X to LDS ping (1x) ; barrier
                #
                # Then per-ni epilogue: each ni runs the same cshuffle skeleton
                # but with its own (by, by_n, acc) so global atomics land on
                # the right output columns.
                #
                # All other helpers (X load, A-scale load, A LDS swizzle, MFMA
                # path, epilogue helpers) are reused unchanged via the optional
                # `n_blk_p / n_intra_p / by_n_p / n_scale_shift_p` parameters
                # added to `load_b_*` / `compute_tile` above.
                # ====================================================================
                if npb_inner > 1:
                    # ---- Per-ni N-state lists (length == npb_inner) ----
                    _npbi_c = arith.constant(int(npb_inner), index=True)
                    _by_outer_scaled_inner = by_outer * _npbi_c
                    _by_list_in = []
                    _by_n_list_in = []
                    _n_intra_list_per_ni = []
                    _n_blk_list_per_ni = []
                    _n_scale_shift_list_in = []
                    for _ni_inner in range_constexpr(int(npb_inner)):
                        _c_ni = arith.index(int(_ni_inner))
                        _by_ni = _by_outer_scaled_inner + _c_ni
                        _by_n_ni = _by_ni * arith.constant(int(tile_n), index=True)
                        _by_list_in.append(_by_ni)
                        _by_n_list_in.append(_by_n_ni)

                        # n_blk / n_intra per ni (one entry per num_acc_n)
                        _ni_intra = [None] * num_acc_n
                        _ni_blk = [None] * num_acc_n
                        for i in range_constexpr(num_acc_n):
                            offset = i * 16
                            c_offset = arith.constant(offset, index=True)
                            _global_n = (
                                _by_n_ni + n_tile_base + c_offset + lane_mod_16
                            )
                            _ni_blk[i] = _div_pow2(_global_n, 16)
                            _ni_intra[i] = _mod_pow2(_global_n, 16)
                        _n_intra_list_per_ni.append(_ni_intra)
                        _n_blk_list_per_ni.append(_ni_blk)

                        # _n_scale_shift per ni (used when pack_N < _scale_pack_n)
                        if pack_N < _scale_pack_n:
                            _g_n_base = (
                                expert_off_idx + _by_n_ni + n_tile_base
                            )
                            _n_off = _mod_pow2(
                                _div_pow2(_g_n_base, 16), _scale_pack_n
                            )
                            _nss_ni = arith.index_cast(
                                T.i32, _n_off * arith.constant(8, index=True)
                            )
                        else:
                            _nss_ni = None
                        _n_scale_shift_list_in.append(_nss_ni)

                    # ---- LDS bases (same as legacy ping-pong) ----
                    lds_tile_elems = arith.constant(
                        tile_m * lds_stride, index=True
                    )
                    lds_base_cur_in = arith.index(0)
                    lds_base_nxt_in = lds_tile_elems

                    rocdl.sched_barrier(0)

                    # ---- Prologue: X-load once, B/B-scale per ni ----
                    if _is_splitk:
                        k0 = _bz_k_off_elems
                        k0_bk = k0 // arith.constant(
                            int(a_elem_vec_pack), index=True
                        )
                    else:
                        k0 = arith.index(0)
                        k0_bk = k0
                    _b_pong_per_ni = [None] * int(npb_inner)
                    _b_scale_pong_per_ni = [None] * int(npb_inner)
                    _prol_scale_base_in = _k_base(0)
                    if _is_splitk:
                        _prol_scale_base_in = (
                            _prol_scale_base_in + _bz_scale_off
                        )
                    # A-scale: N-INDEP, single load shared across all ni.
                    _a_scale_pong_in = load_a_scale_tile(
                        _prol_scale_base_in, _k_shift_bits(0)
                    )
                    for _ni_inner in range_constexpr(int(npb_inner)):
                        _nblk = _n_blk_list_per_ni[_ni_inner]
                        _nint = _n_intra_list_per_ni[_ni_inner]
                        if _b_split_enabled:
                            _b_pong_per_ni[_ni_inner] = load_b_tile_lo(
                                k0_bk, n_blk_p=_nblk, n_intra_p=_nint
                            )
                        else:
                            _b_pong_per_ni[_ni_inner] = load_b_tile(
                                k0_bk, n_blk_p=_nblk, n_intra_p=_nint
                            )
                        _b_scale_pong_per_ni[_ni_inner] = load_b_scale_tile(
                            _prol_scale_base_in,
                            _k_shift_bits(0),
                            by_n_p=_by_n_list_in[_ni_inner],
                        )
                    # Make scale loads stick before the upcoming barrier.
                    rocdl.sched_barrier(0)

                    x_regs0_in = load_x_tile(k0)
                    store_x_tile_to_lds(x_regs0_in, lds_base_cur_in)
                    gpu.barrier()

                    # Per-ni accumulators.
                    _acc_per_ni = [
                        [acc_init] * num_acc_n * m_repeat
                        for _ in range(int(npb_inner))
                    ]
                    lds_base_pong_in = lds_base_cur_in
                    lds_base_ping_in = lds_base_nxt_in

                    # Cross-tile A LDS prefetch (N-INDEP, shared across ni).
                    _a0_prefetch_pong_in = lds_load_packs_k64(
                        row_a_lds, col_offset_base, lds_base_pong_in
                    )
                    _a1_col_base_in = (
                        col_offset_base + 128 // a_elem_vec_pack
                    )
                    _a1_prefetch_pong_in = (
                        lds_load_packs_k64(
                            row_a_lds, _a1_col_base_in, lds_base_pong_in
                        )
                        if pack_K >= 2
                        else None
                    )

                    # ---- Main loop: 2-tile ping-pong over K ----
                    num_k_tiles_py = _num_k_tiles_per_batch
                    odd_k_tiles = (num_k_tiles_py % 2) == 1
                    tail_tiles = 1 if odd_k_tiles else 2
                    k_main2_py = (
                        num_k_tiles_py - tail_tiles
                    ) * int(tile_k)
                    if k_main2_py < 0:
                        k_main2_py = 0
                    c2_tile_k_in = arith.constant(tile_k * 2, index=True)

                    # b_hi loaders are also per-ni now
                    _k0_pong_bk_in = k0_bk

                    def _make_b_hi_loader_ni(base_k, _ni_inner):
                        _nblk = _n_blk_list_per_ni[_ni_inner]
                        _nint = _n_intra_list_per_ni[_ni_inner]
                        return lambda _bk=base_k, _b=_nblk, _i=_nint: load_b_tile_hi(
                            _bk, n_blk_p=_b, n_intra_p=_i
                        )

                    if k_main2_py > 0:
                        for k_iv_py in range_constexpr(
                            0, k_main2_py, tile_k * 2
                        ):
                            if _is_splitk:
                                k_iv = (
                                    arith.index(k_iv_py) + _bz_k_off_elems
                                )
                            else:
                                k_iv = arith.index(k_iv_py)
                            next_k1 = k_iv + tile_k
                            next_k1_bk = next_k1 // 2
                            # X (A) and A-scale next-K loads -- N-INDEP (1x).
                            x_regs_ping_in = load_x_tile(next_k1)
                            _a_scale_ping_in = load_a_scale_tile(
                                _k_base(next_k1), _k_shift_bits(next_k1)
                            )
                            # Per-ni B-lo + B-scale prefetch for next K.
                            _b_ping_lo_per_ni = [None] * int(npb_inner)
                            _b_scale_ping_per_ni = [None] * int(
                                npb_inner
                            )
                            for _ni_inner in range_constexpr(int(npb_inner)):
                                _nblk = _n_blk_list_per_ni[_ni_inner]
                                _nint = _n_intra_list_per_ni[_ni_inner]
                                _b_ping_lo_per_ni[_ni_inner] = (
                                    load_b_tile_lo(
                                        next_k1_bk,
                                        n_blk_p=_nblk,
                                        n_intra_p=_nint,
                                ...
                                _b_scale_ping_per_ni[_ni_inner] = (
                                    load_b_scale_tile(
                                        _k_base(next_k1),
                                        _k_shift_bits(next_k1),
                                        by_n_p=_by_n_list_in[_ni_inner],
                                    )
                                )

                            # Compute current (pong) K-tile per ni.
                            for _ni_inner in range_constexpr(int(npb_inner)):
                                _bhi = (
                                    _make_b_hi_loader_ni(
                                        _k0_pong_bk_in, _ni_inner
                                    )
                                    if _b_split_enabled
                                    else None
                                )
                                _acc_per_ni[_ni_inner], _ = compute_tile(
                                    _acc_per_ni[_ni_inner],
                                    _b_pong_per_ni[_ni_inner],
                                    lds_base_pong_in,
                                    _a_scale_pong_in,
                                    _b_scale_pong_per_ni[_ni_inner],
                                    a0_prefetch=_a0_prefetch_pong_in,
                                    a1_prefetch=_a1_prefetch_pong_in,
                                    b_hi_loader=_bhi,
                                    n_scale_shift_p=_n_scale_shift_list_in[
                                        _ni_inner
                                    ],
                                )
                            store_x_tile_to_lds(
                                x_regs_ping_in, lds_base_ping_in
                            )
                            gpu.barrier()

                            # Cross-tile A LDS prefetch for ping (shared).
                            _a0_prefetch_ping_in = lds_load_packs_k64(
                                row_a_lds,
                                col_offset_base,
                                lds_base_ping_in,
                            )
                            _a1_prefetch_ping_in = (
                                lds_load_packs_k64(
                                    row_a_lds,
                                    _a1_col_base_in,
                                    lds_base_ping_in,
                            ...
                            # Now load second K-tile (pong-next) X / A-scale
                            # / per-ni B / per-ni B-scale.
                            next_k2 = k_iv + c2_tile_k_in
                            next_k2_py = k_iv_py + tile_k * 2
                            next_k2_bk = next_k2 // 2
                            x_regs_pong_in = load_x_tile(next_k2)
                            _nk2_scale_base = _k_base(next_k2_py)
                            if _is_splitk:
                                _nk2_scale_base = (
                                    _nk2_scale_base + _bz_scale_off
                                )
                            _a_scale_pong_in = load_a_scale_tile(
                                _nk2_scale_base,
                                _k_shift_bits(next_k2_py),
                            )
                            for _ni_inner in range_constexpr(int(npb_inner)):
                                _nblk = _n_blk_list_per_ni[_ni_inner]
                                _nint = _n_intra_list_per_ni[_ni_inner]
                            ...
                            # Compute ping K-tile per ni.
                            for _ni_inner in range_constexpr(int(npb_inner)):
                                _bhi = (
                                    _make_b_hi_loader_ni(
                                        next_k1_bk, _ni_inner
                                    )
                                    if _b_split_enabled
                                    else None
                                )
                                _acc_per_ni[_ni_inner], _ = compute_tile(
                                    ...
                                    _a_scale_ping_in,
                                    _b_scale_ping_per_ni[_ni_inner],
                                    a0_prefetch=_a0_prefetch_ping_in,
                                    a1_prefetch=_a1_prefetch_ping_in,
                                    b_hi_loader=_bhi,
                                    n_scale_shift_p=_n_scale_shift_list_in[
                                        _ni_inner
                                    ],
                                )
                            _k0_pong_bk_in = next_k2_bk
                            store_x_tile_to_lds(
                                x_regs_pong_in, lds_base_pong_in
                            )
                            gpu.barrier()
                    ...
                    # ---- Tail ----
                    if odd_k_tiles:
                        # Single remaining tile (already in `_b_pong_per_ni` /
                        # `lds_base_pong_in`).
                        for _ni_inner in range_constexpr(int(npb_inner)):
                            _bhi = (
                                _make_b_hi_loader_ni(
                        ...
                        # 2-tile tail.
                        if _is_splitk:
                            _tail_k_rel = _k_per_batch - int(tile_k)
                            k_tail1_py = _tail_k_rel
                            k_tail1 = (
                                arith.constant(_tail_k_rel, index=True)
                                + _bz_k_off_elems
                            )
                        else:
                            k_tail1 = (
                                (k_in + tile_k - 1) // tile_k * tile_k
                                - tile_k
                            )
                            k_tail1_py = (
                        ...
                        x_regs_ping_in = load_x_tile(k_tail1)
                        _tail_scale_base = _k_base(k_tail1_py)
                        if _is_splitk:
                            _tail_scale_base = (
                                _tail_scale_base + _bz_scale_off
                            )
                        _a_scale_ping_in = load_a_scale_tile(
                            _tail_scale_base, _k_shift_bits(k_tail1_py)
                        )
                        _b_ping_lo_per_ni = [None] * int(npb_inner)
                        _b_scale_ping_per_ni = [None] * int(npb_inner)
                        for _ni_inner in range_constexpr(int(npb_inner)):
                            _nblk = _n_blk_list_per_ni[_ni_inner]
                            _nint = _n_intra_list_per_ni[_ni_inner]
                            _b_ping_lo_per_ni[_ni_inner] = (
                        ...
                        # Compute ping (final) tile.
                        for _ni_inner in range_constexpr(int(npb_inner)):
                            _bhi = (
                                _make_b_hi_loader_ni(
                                    k_tail1_bk, _ni_inner
                                )
                                if _b_split_enabled
                                else None
                            )
                    ...
                    # ---- Per-ni Epilogue: cshuffle + atomic ----
                    # Run sequentially per-ni so each writes to its own
                    # output slice. We rebind the ``acc`` / ``by_n`` /
                    # ``n_tile_base``-derived state via Python locals that
                    # the helpers below close over.
                    _run_per_ni_epilogue_inline(
                        npb_inner=int(npb_inner),
                        acc_per_ni=_acc_per_ni,
                        by_list=_by_list_in,
                        by_n_list=_by_n_list_in,
                        n_tile_base=n_tile_base,
                    )
                    return
                ...
                def hot_loop_scheduler():
                    ...
                    # Team D r14 (dswr_async): with use_async_copy=True the X side
                    # uses raw_ptr_buffer_load_lds whose LDS write is internal to
                    # the buffer-load — no register-mediated ds_write exists.
                    # The legacy dswr_tail = num_x_loads overcounts, biasing the
                    # scheduler.  Zero it for async; sync path unchanged.
                    if use_async_copy:
                        dswr_tail = 0
                    else:
                ...
                #
                # Round 12 optimization: for the buffer-atomic prefill path
                # (`_use_buf_atomic_pre`), pre-compute the per-row byte offset
                # `(t & 0xFFFFFF) * row_stride_bytes` and pre-OOB-mask invalid
                # rows here in the prologue (where the work overlaps with the
                # B/scale/X VMEM loads). The cshuffle epilogue's
                # `precompute_row` then becomes a single LDS load — no AND, no
                # multiply, no select on the per-row critical path. Saves ~5
                # v_alu ops per cshuffle row (8 rows/epilogue).
                ...
                _c_tile_m_idx = arith.constant(tile_m, index=True)
                _tid_in_range = arith.cmpi(CmpIPredicate.ult, tx, _c_tile_m_idx)
                # Round 216 (Team A): defer the lds_tid prologue (sorted_idx
                # /sorted_w VMEM dwords + LDS stores) to AFTER the long-latency
                # VMEM loads (X-LDS DMA + B-tile load + scale prefetch). The
                # lds_tid IfOp's two scalar dword buffer_loads then issue while
                # the X DMA and B/scale loads are already in flight, hiding
                # their latency behind the dominant VMEM phase. Result is
                # consumed only by the cshuffle epilogue precompute_row, so
                # any LDS visibility is provided by the post-prologue
                # `gpu.barrier()` (which already syncs lds_x and lds_b too).
                # Gated to the production prefill path so other configs are
                # bit-identical.
                ...
                def _emit_tid_lds_prologue():
                    # Round 58 (Team A): merge the lds_tid and lds_tw prologue
                    # store guards into a single scf.IfOp.  Both check identical
                    # `tx < tile_m` predicates and write per-thread to LDS; merging
                    # them eliminates one branch, reduces basic-block count and
                    # lets MLIR co-schedule the two VMEM buffer_loads (sorted_rsrc
                    # + sorted_w_rsrc) so they issue back-to-back, allowing the
                    # second VMEM op to overlap with address-decode for the first
                    # rather than waiting for the IfOp boundary to close and reopen.
                    _if_tid = scf.IfOp(_tid_in_range)
                    with ir.InsertionPoint(_if_tid.then_block):
                        _tid_row = bx_m + tx
                        _tid_val = buffer_ops.buffer_load(
                        ...
                        if _use_buf_atomic_pre:
                            # _OOB_safe_i32 (0x7FFF0000) matches the constant used
                            # downstream in the cshuffle epilogue and is safely past
                            # out_rsrc.num_records_bytes — HW silently drops the
                            # atomic for OOB voffset.
                            _mask24_pre = arith.constant(0xFFFFFF, type=T.i32)
                            _stride_pre = arith.constant(
                                _row_stride_bytes_pre, type=T.i32
                            )
                            _oob_pre = arith.constant(0x7FFF0000, type=T.i32)
                            _t_pre = _tid_val & _mask24_pre
                            _s_pre = arith.shrui(
                                _tid_val, arith.constant(24, type=T.i32)
                            )
                            _row_byte_off_pre = _t_pre * _stride_pre
                            _global_row_i32 = arith.index_cast(T.i32, _tid_row)
                            _row_valid_pre0 = arith.cmpi(
                                CmpIPredicate.ult, _global_row_i32, num_valid_i32
                            )
                            ...
                            _row_valid_pre = arith.andi(
                                arith.andi(_row_valid_pre0, _t_valid_pre),
                                _s_valid_pre,
                            )
                            _stored_val = arith.select(
                                _row_valid_pre, _row_byte_off_pre, _oob_pre
                            )
                        else:
                ...
                # NOTE: removed the post-lds_tid `gpu.barrier()` here. lds_tid
                # is read only in the cshuffle epilogue (precompute_row), which
                # runs AFTER many K-loop gpu.barriers — the next prologue
                # `gpu.barrier()` (at the end of B/X prefetch) plus those K-loop
                # barriers all sync lds_tid for the consumers. The eliminated
                # barrier was redundant and was a per-persistent-iteration cost.
                # `prefetch_x_to_lds` writes to lds_x bytes (per-wave region)
                # and does not depend on lds_tid contents.

                # Prologue -- B-first.
                # For split-K: start this block's K range at bz * _k_per_batch.
                # (Keep the non-split path on a bare arith.index(0) so the
                # emitted MLIR is bit-identical to the legacy kernel.)
                #
                # Unit note: `k0` is in **A elements** (used by load_x_tile).
                # B loaders expect base_k in **packed bytes** (= elements /
                # a_elem_vec_pack for fp4). For the non-split path k0=0 so both
                # units alias; for split-K we must divide by a_elem_vec_pack
                # before handing to any B loader.
                if _is_splitk:
                    k0 = _bz_k_off_elems
                    k0_bk = k0 // arith.constant(int(a_elem_vec_pack), index=True)
                else:
                    k0 = arith.index(0)
                    k0_bk = k0
                if _r139_xdma_first:
                ...
                _prol_scale_base = _k_base(0)
                if _is_splitk:
                    _prol_scale_base = _prol_scale_base + _bz_scale_off
                a_scale_pong, b_scale_pong = prefetch_ab_scale_tile(
                    _prol_scale_base, _k_shift_bits(0)
                )
                rocdl.sched_barrier(0)
                if not _r139_xdma_first:
                    ...
                    store_x_tile_to_lds(x_regs0, lds_base_cur)
                if _r216_defer_tid:
                    # Issue the lds_tid prologue *after* the heavy VMEM phase
                    # (X DMA + B-tile + scale prefetch) so the two scalar
                    # buffer_loads inside the IfOp issue while the long-latency
                    # ops are still in flight.  The post-prologue gpu.barrier
                    # below already syncs lds_tid for the cshuffle epilogue.
                    _emit_tid_lds_prologue()
                    rocdl.sched_barrier(0)
                gpu.barrier()
                ...
                # For split-K, each grid.z slice only covers _k_per_batch of K;
                # our split-K path enforces even num_k_tiles_per_batch >= 2, so
                # tail_tiles is always 2 in that mode (no odd-tail path).
                num_k_tiles_py = _num_k_tiles_per_batch
                odd_k_tiles = (num_k_tiles_py % 2) == 1
                tail_tiles = 1 if odd_k_tiles else 2
                k_main2_py = (num_k_tiles_py - tail_tiles) * int(tile_k)
                ...
                if k_main2_py > 0:
                    for k_iv_py in range_constexpr(0, k_main2_py, tile_k * 2):
                        # Shift this block's relative K by the grid.z batch
                        # offset. Guarded to keep the non-split path MLIR
                        # bit-identical to the legacy emission.
                        if _is_splitk:
                            k_iv = arith.index(k_iv_py) + _bz_k_off_elems
                        else:
                            k_iv = arith.index(k_iv_py)
                        next_k1 = k_iv + tile_k
                        next_k1_bk = next_k1 // 2
                        # R30: issue scales BEFORE B-VMEM in the steady-state
                        # K-loop. Scales are 1 dword per chunk (small VMEM
                        # latency) and feed `vector.extract` at the very
                        # head of compute_tile's MFMA loop (a_scale_val /
                        # b_scale_val are read per (ku128, mi/ni) before
                        # the first MFMA in that group). Issuing them ahead
                        # of the longer B cache-line loads (vec_width 1,
                        # multiple per (ku, ni)) lets the scalar values
                        # land first while the B loads continue to overlap
                        # MFMA latency below. The post-compute_tile
                        # gpu.barrier() still waits on vmcnt(0) for both,
                        # so total issue cost is unchanged but
                        # extract-time stalls in the first MFMA chunk
                        # are reduced.
                        if use_async_copy:
                        ...
                        next_k2_py = k_iv_py + tile_k * 2
                        next_k2_bk = next_k2 // 2
                        # R30: scales-before-B (see above for rationale).
                        if use_async_copy:
                            prefetch_x_to_lds(next_k2, lds_base_pong)
                        else:
                            x_regs_pong = load_x_tile(next_k2)
                        # Scale base: Python-time constant folds the within-
                        # batch offset; add the MLIR bz scale-pack offset for
                        # split-K (baseline emits bit-identical MLIR).
                        _nk2_scale_base = _k_base(next_k2_py)
                        if _is_splitk:
                            _nk2_scale_base = _nk2_scale_base + _bz_scale_off
                        a_scale_pong, b_scale_pong = prefetch_ab_scale_tile(
                            _nk2_scale_base, _k_shift_bits(next_k2_py)
                        )
                        b_pong = (
                ...
                if odd_k_tiles:
                    ...
                    # For split-K, bound is _k_per_batch (batch-relative).  We
                    # use a compile-time constant here instead of runtime k_in
                    # because k_batch is a compile-time parameter and we've
                    # already validated divisibility above.
                    if _is_splitk:
                        _tail_k_rel = _k_per_batch - int(tile_k)  # Python int
                        k_tail1_py = _tail_k_rel
                        k_tail1 = (
                            arith.constant(_tail_k_rel, index=True)
                            + _bz_k_off_elems
                    ...
                    # Scale base_k = relative scale pack index + per-batch step
                    # (MLIR add is fine since Python-int + MLIR-index promotes).
                    _tail_scale_base = _k_base(k_tail1_py)
                    if _is_splitk:
                        _tail_scale_base = _tail_scale_base + _bz_scale_off
                    a_scale_ping, b_scale_ping = prefetch_ab_scale_tile(
                        _tail_scale_base, _k_shift_bits(k_tail1_py)
                    )
                ...
                # Constant safely beyond out_rsrc.num_records_bytes for the
                # buffer-atomic path. We add ≤ tile_n*out_elem_bytes (~256
                # bytes) of column offset on top, so 0x7FFF0000 is far enough
                # below INT32_MAX to never wrap, and far above the largest
                # plausible num_records_bytes (~470 MB for prefill DSV3
                # accumulate=True). HW silently drops OOB buffer atomics, so
                # routing invalid rows here lets us drop the per-mr scf.IfOp
                # branch in the cshuffle epilogue.
                _OOB_safe_i32 = (
                ...
                def precompute_row(*, row_local, row):
                    # Round 12: for the buffer-atomic path the lds_tid preload
                    # already stored `(t & 0xFFFFFF) * row_stride_bytes` with
                    # invalid rows masked to _OOB_safe_i32. So this fast path
                    # is just one ds_read — no AND, no multiply, no validity
                    # cmpi/select on the per-row critical path. Cuts ~5 v_alu
                    # ops per cshuffle row × 8 rows × multiple epilogues per
                    # CTA. The work was moved up to the lds_tid preload site
                    # where it overlaps with B/scale/X VMEM loads.
                    if _use_buf_atomic:
                ...
                def store_pair(*, row_local, row, row_ctx, col_pair0, col_g0, frag):
                    ...
                    if not bool(accumulate):
                        ...
                        # ---- accumulate=True: buffer-atomic path ----
                        # Lowers to BUFFER_ATOMIC_PK_ADD_BF16. The buffer
                        # descriptor (out_rsrc) holds the 64-bit base in
                        # SGPRs, leaving only the 32-bit voffset in VGPR.
                        # This reduces VGPR pressure on the address vs
                        # llvm.AtomicRMWOp (which lowers to global_atomic
                        # and materializes a full 64-bit pointer per op).
            ...
            # Inner N-tile loop: each block covers `n_per_block` consecutive
            # N-tiles. To keep register pressure constant across N iterations
            # we lower the loop as an *scf.for* (i.e. a runtime loop) rather
            # than a python-time unroll. Values produced across the K-loop
            # accumulator phase thus get re-used register storage between N
            # iters, avoiding the VGPR inflation that a full unroll incurs.
            if n_per_block > 1:
                _npb_c_pre = arith.constant(n_per_block, index=True)
                _by_outer_scaled = by_outer * _npb_c_pre

            def _run_body_for_all_n():
                # `by` lives in the enclosing moe_gemm2 scope and is read by
                # _moe_gemm2_then_body when it computes `by_n = by * tile_n`.
                # For n_per_block == 1 the assignment is trivial. Otherwise
                # emit an scf.for and rebind `by` inside the loop body.
                nonlocal by
                if n_per_block == 1:
                    by = by_outer
                    _moe_gemm2_then_body()
                    return
                _for_n = scf.ForOp(_c0_p, _npb_c_pre, _c1_p)
                _for_n_ip = ir.InsertionPoint(_for_n.body)
                _for_n_ip.__enter__()
                _ni = _for_n.induction_variable
                by = _by_outer_scaled + _ni
                _moe_gemm2_then_body()
                scf.YieldOp([])
                _for_n_ip.__exit__(None, None, None)
    ...
    def launch_mixed_moe_gemm2(
        ...
        # With inner N-tile loop, each block covers `n_per_block` N-tiles ->
        # grid shrinks by the same factor.
        gx = n_in / arith.constant(tile_n * n_per_block * npb_inner, index=True)
        if _persistent:
